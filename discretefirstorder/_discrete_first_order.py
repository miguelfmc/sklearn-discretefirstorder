...
# TODO move to different branch - not ready yet
# (can we even apply this method? no closed form solution for logistic regression)
class DFOClassifier(BaseEstimator, ClassifierMixin):
    """Discrete First-Order Classifier.

    Parameters
    ----------
    loss : str
        loss function to be used. Currently only "bce" is supported.
    ...
    def __init__(
        ...
        super(DFOClassifier, self).__init__(
            loss=loss,
            learning_rate=learning_rate,
            k=k,
            polish=polish,
    ...
    # TODO consider whether _set_intercept method can be shared with DFORegressor
    def _set_intercept(self, X_offset, y_offset, X_scale):
        """Set intercept.
    ...
    # TODO consider whether fit method can be shared with DFORegressor
    def fit(self, X, y):
    ...
    def predict_proba(self, X):
        """Implementation of a probability prediction for the discrete first-order classifier."""
        # check is fit had been called
        check_is_fitted(self, ["coef_", "intercept_"])
